...
private _type = getText(configFile >> "CfgTaskTypes" >> _configName >> "icon");

private _taskID = format["%1_%2", QGVAR(task),GVAR(task_counter) + 1];
GVAR(task_counter) = GVAR(task_counter) + 1;

[_taskID, _side, [_description, _name, _marker], _destination select 1, "AUTOASSIGNED", 1, false, true, _type, false] call BIS_fnc_setTask;
