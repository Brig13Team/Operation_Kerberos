...
private _allMissionTargets = [] call FUNC(getAllMissionTargets);

[
    15,
    [_checker,_checkerPositions,_allMissionTargets],
    {
        [_args select 0,_args select 2] spawn FUNC(obj_effectTriangulation);
    },
    {
        [LSTRING(OBJ_TRIANGULATE_MOVED_MSG_TITLE),LSTRING(OBJ_TRIANGULATE_MOVED_MSG)] call EFUNC(gui,Message);
    },
    localize LSTRING(OBJ_DATATERMINAL_TRIANGULATING),
    {
        private _check = true;
        {
