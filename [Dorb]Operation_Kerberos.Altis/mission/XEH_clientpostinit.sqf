...
///// suitcase actions
private _ACE_Action = [QGVAR(objects_carry), localize LSTRING(OBJECTS_CARRY), "", {[_target,player] spawn FUNC(objects_carry);}, {(isNull (_target getVariable [QGVAR(objects_carrier),objNull]))}] call ace_interact_menu_fnc_createAction;
