...
switch (_mode) do {
    case "check" : {
        ...
        private _mass_new = ["getmass",[_config,_amount]] call FUNC(crate_mass);
        private _mass_current = ["currentmass"] call FUNC(crate_mass);
        private _mass_max = (GVAR(crate_boxes) select 2) select GVAR(crate_current_boxid);
        (!((_mass_current+_mass_new)<_mass_max));
    };
    case "currentmass" : {
        private _mass_current = 0;
        for "_i" from 0 to ((count (GVAR(crate_current) select 0))-1) do {
            private _mass = ["getmass",((GVAR(crate_current) select 0)select _i)] call FUNC(crate_mass);
        ...
        for "_i" from 0 to ((count (GVAR(crate_current) select 2))-1) do {
            private _mass = ["getmass",((GVAR(crate_current) select 2)select _i)] call FUNC(crate_mass);
