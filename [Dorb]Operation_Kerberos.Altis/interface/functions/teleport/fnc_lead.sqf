...
_time = GETMVAR(GVAR(respawntime),0);

if ((_time > 0) && {(_time + 300) > diag_tickTime}) exitWith {[LSTRING(TELEPORT),[format [localize LSTRING(TELEPORT_LEAD_WAIT),round (_time + 300 - diag_tickTime)]]] call FUNC(disp_info);};
