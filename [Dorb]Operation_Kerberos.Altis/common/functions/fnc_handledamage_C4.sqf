...
switch (true) do {
        default { 0 };
        case ( (_this select 4) == "demoCharge_remote_ammo") : {SETVAR((_this select 0),EGVAR(mission,target_dead),true); (getDammage (_this select 0) + 0.51) min 1; };
        case ( (_this select 4) == "DemoCharge_Remote_Ammo_Scripted") : {SETVAR((_this select 0),EGVAR(mission,target_dead),true); (getDammage (_this select 0) + 0.51) min 1; };
