...
If (_message isEqualTo "") exitWith {
	[format["[%1_%2_%3:%4]-%5 (%6:%7) ERROR-LOGAUFRUF",_addon,_part,_file,_line,_time,diag_tickTime,diag_frameNo]] call FUNC(debug_write);
};
private["_lines","_time"];
_lines = [_message,"\n"] call CBA_fnc_split;
...
if (!(_file isEqualTo "")) then {
	...
	[format["[%1_fnc_%2_%3:%4]-%5 (%6:%7) %8",_addon,_part,_file,_line,_time,diag_tickTime,diag_frameNo,_header]] call FUNC(debug_write);
}else{
...
If (isServer&&hasinterface&&_header in ["ERROR","WARNING","TRACE"]) then {
	[format["[%1_fnc_%2_%3:%4]-%5 (%6:%7) %8",_addon,_part,_file,_line,_time,diag_tickTime,diag_frameNo,(_lines deleteAt 0)]] call FUNC(debug_show);
	{
