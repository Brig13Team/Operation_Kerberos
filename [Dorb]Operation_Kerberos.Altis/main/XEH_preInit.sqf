...
TRIPLES(PREFIX,makro,iscasvehicle) = {
    ...
    if ((isClass(configFile >> "CfgVehicles" >> _class)) && {("CAS_Heli" in getArray(configFile >> "CfgVehicles" >> _class >> "availableForSupportTypes"))}) exitWith { true };
    if ((isClass(configFile >> "CfgVehicles" >> _class)) && {"CAS_Bombing" in getArray(configFile >> "CfgVehicles" >> _class >> "availableForSupportTypes")}) exitWith { true };
    false
};
