...
private _attackarray = [];
{
    ...
    private _currentTroops = _currentLocation getVariable QGVAR(troopsSend);
    If (isNil "_currentTroops") then {
        _currentTroops = _currentEnemy;
        _currentLocation setVariable [QGVAR(troopsSend),_currentTroops];
    };
...
{
    (_attackarray select _i) params ["_currenttroopsSend","_currentEnemy","_currentLocation","_type","_strengtharray"];
    private _currentStrategy = _currentLocation getVariable [QGVAR(strategy),[]];
    If (_currentStrategy isEqualTo []) then {
        [_currenttroopsSend,_currentEnemy,_currentLocation,_type,_strengtharray,_groups] call FUNC(strategy__choose);
    }else{
        // strategy failed
        If ((_currentEnemy*0.2)>_currenttroopsSend) then {
