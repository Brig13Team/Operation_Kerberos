...
if ((count allPlayers)>5) exitWith {["STR_DORB_HALO","STR_DORB_HALO_DISABLED"] call EFUNC(interface,disp_message);};

if !(alive _host) exitwith {
	["STR_DORB_HALO","STR_DORB_HALO_DISABLED"] call EFUNC(interface,disp_message);
	_host removeaction _id;
};

if (vehicle _caller == _caller) then {
	private ["_pos"];
	["STR_DORB_HALO","STR_DORB_HALO_KlICK"] call EFUNC(interface,disp_message);
	openMap true;
	mapclick = false;
	onMapSingleClick "clickpos = _pos; mapclick = true; onMapSingleClick """";true;";
	waituntil {mapclick or !(visiblemap)};
	if !(visibleMap) exitwith {
		["STR_DORB_HALO","STR_DORB_HALO_ABORT"] call EFUNC(interface,disp_message);
	};
	_pos = clickpos;
