...
If ((!(_notifications isEqualTo []))||{({(!(_x isEqualTo [0,0,0]))&&{(player distance _x)<200}} count [getMarkerPos GVARMAIN(respawnmarker),getMarkerPos "respawn_west_tonos"])>0}) then {
