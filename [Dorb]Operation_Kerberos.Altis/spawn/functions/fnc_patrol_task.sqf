...
_pos = switch (true) do {
    ...
    case ((typeOf (leader _group))in (["divers"] call EFUNC(spawn,getUnit))): {_temp = [_centerpos,1000,3] call EFUNC(common,pos_random);{_x swimInDepth -10;} forEach (units _group);_temp set [2,-10];_temp};
