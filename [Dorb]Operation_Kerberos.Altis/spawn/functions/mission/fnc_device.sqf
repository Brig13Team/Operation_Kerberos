...
private _targetPositions = [_centerposition,["device","isObjective"],1,_radius] call FUNC(createMissionComposition);
private _targetPos = selectRandom (_targetPositions select 0);

private _obj = ["device"] call FUNC(getMissionObject);
