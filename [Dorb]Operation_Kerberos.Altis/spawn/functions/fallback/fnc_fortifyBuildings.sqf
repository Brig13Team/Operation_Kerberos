...
while {(_amountDefHouses > 0)&&(count _allHouses > 0)} do {
    ...
    If (isClass(missionConfigFile >> "CfgCompositions" >> "housepositions" >> _curType)) then {
        private _soldierPositions = getArray(missionConfigFile >> "CfgCompositions" >> "housepositions" >> _curType >> "soldierpos");
