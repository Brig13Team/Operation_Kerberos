...
CHECK(!(isClass (missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit))))
private ["_uniform","_uniformitems","_backpack","_backpackitems","_vest","_vestitems"];
_uniform = gettext(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"uniform");
_uniformitems = getArray(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"uniformitems");
...
_vest = gettext(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"vest");
_vestitems = getArray(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"vestitems");
if !(_uniform isEqualTo "") then {
...
_backpack = gettext(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"vest");
_backpackitems = getArray(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"vestitems");
if !(_uniform isEqualTo "") then {
...
_primaryweapon = gettext(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"primaryweapon");
If !(_primaryweapon isEqualTo []) then {
	_unit removeWeapon (primaryweapon _unit);
	_unit addWeapon _primaryweapon;
	_unit addMagazine (primaryWeaponMagazine _unit);
};
_primaryweaponattachments = getarray(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"primaryweaponattach");
If !(_primaryweaponattachments isEqualTo "") then {
	removeAllPrimaryWeaponItems _unit;
	{_unit addPrimaryWeaponItem _x;}forEach _primaryweaponattachments;
};

_secondaryweapon = gettext(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"secondaryweapon");
If !(_secondaryweapon isEqualTo []) then {
	_unit removeWeapon (secondaryweapon _unit);
	_unit addWeapon _secondaryweapon;
	_unit addMagazine (secondaryWeaponMagazine _unit);
};
_secondaryweaponattachments = getarray(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"secondaryweaponattach");
...
_handgun = gettext(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"handgun");
If !(_handgun isEqualTo "") then {
	_unit removeWeapon (handgunweapon _unit);
	_unit addWeapon _handgun;
	_unit addMagazine (handgunMagazine _unit);
};
_handgunattachments = getarray(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"handgunattach");
...
_linkeditems = getarray(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"linkeditems");
If !(_linkeditems isEqualTo []) then {
	{_unit linkItem _x;}forEach _linkeditems;
};

_headgear = gettext(missionconfigFile>>"gear">>QGVARMAIN(side_type)>>(typeOf _unit)>>"headgear");
