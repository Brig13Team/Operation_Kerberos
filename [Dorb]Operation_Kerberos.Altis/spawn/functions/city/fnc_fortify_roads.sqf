...
for [{_i = 0},{_i <= (_amounts select 1)},{_i = _i + 1;}] do {
	...
	_add_pos = -1;
	_id = [_roadArray_weighted,0,true] call EFUNC(common,sel_array_weighted);
...
If (_fortifications) then {
	{
		_x call FUNC(city_fortify_roads_defence);
...
If (_IEDs) then {
	...
	[_IED_array,3] call FUNC(city_spawn_explosives);
};
/// Civ-Vehicles
