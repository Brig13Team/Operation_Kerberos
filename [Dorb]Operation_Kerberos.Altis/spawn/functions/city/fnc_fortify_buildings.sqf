...
for "_i" from 0 to ((count _buildings)-1) do {
    ...
    If (!(_buildingpos isEqualTo [])) then {
        ...
        If ((_spawnamount>0)&&(_staticpos isEqualTo [])) then {
            [_gruppe,(getPos (leader _gruppe))] call FUNC(city_patrol_building);
        };
...
If (!(_mines_spawnarray isEqualTo [])) then {
    [_mines_spawnarray,2] call FUNC(city_spawn_explosives);
};
