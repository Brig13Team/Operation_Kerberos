...
TRIPLES(PREFIX,makro,delete) = compileFinal "#include 'script_component.hpp'If(IS_ARRAY(_this))exitWith{_x TILGE;}forEach _this;If(IS_GROUP(_this))exitWith{deletegroup _this;};If(IS_OBJECT(_this))exitWith{{_this deleteVehicleCrew _x} forEach crew _this; deleteVehicle _this;};If(IS_STRING(_this))exitWith{deleteMarker _this};ERROR(FORMAT_1('TILE: Nicht unterstuetzt | %1',_this));"
