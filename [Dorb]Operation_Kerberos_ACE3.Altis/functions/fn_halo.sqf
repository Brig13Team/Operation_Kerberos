...
if ((count _players)>5) exitWith {[localize "STR_DORB_HALO",localize "STR_DORB_HALO_DISABLED"] call dorb_fnc_disp_message;};


if (not alive _host) exitwith {
	[localize "STR_DORB_HALO",localize "STR_DORB_HALO_DISABLED"] call dorb_fnc_disp_message;
...
if (vehicle _caller == _caller) then {
	...
	[localize "STR_DORB_HALO",localize "STR_DORB_HALO_KlICK"] call dorb_fnc_disp_message;
	...
	if !(visibleMap) exitwith {
		[localize "STR_DORB_HALO",localize "STR_DORB_HALO_ABORT"] call dorb_fnc_disp_message;
	};
	_pos = clickpos;
	...
	[_caller]spawn{
		...
		_gear=[];
		_gear = [_caller] call dorb_fnc_get_loadout_halo;
		...
		[_caller,_gear] call dorb_fnc_set_loadout_halo;
	};
	*/
