...
_loadAction = ["DORB_LOG_ACTION_LOAD", localize "STR_DORB_LOG_ACTION_LOAD", "", {[_target] spawn dorb_fnc_log_doload;}, {[_target] call dorb_fnc_log_canload}] call ace_interact_menu_fnc_createAction;
_unloadAction = ["DORB_LOG_ACTION_UNLOAD", localize "STR_DORB_LOG_ACTION_UNLOAD", "", {[_target] call FM(log_unload);}, {  (!((_target getVariable ["LOGISTIC_CARGO_STACK",[]]) isEqualTo [] ))  }] call ace_interact_menu_fnc_createAction;
