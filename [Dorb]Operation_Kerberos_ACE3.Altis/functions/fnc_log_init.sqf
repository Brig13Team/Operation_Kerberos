...
_loadAction = ["DORB_LOG_ACTION_LOAD", localize "STR_DORB_LOG_ACTION_LOAD", "", {[_target] call FM(log_load);}, {( (GETVAR(DORB_LOG_FREESPACE,( getnumber(missionconfigFile >> "logistics" >> (typeOf _target) >> "canload"))))&&{[_target] call FM(log_can_load)} )}] call ace_interact_menu_fnc_createAction;
