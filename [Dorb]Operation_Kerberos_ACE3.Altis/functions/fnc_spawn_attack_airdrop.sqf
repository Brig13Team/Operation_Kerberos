...
_position = [_position,100,0] call FM(random_pos);

sleep (random 9);
_spawnpos = [_position,_spawnradius,1] call FM(random_pos);
...
for "_i" from 0 to (_platzanzahl - 2) do {
	...
	_jaeger = [[(_spawnpos select 0),(_spawnpos select 1),0],_jaeger_gruppe,_einheit] call FM(spawn_unit);
	//sleep 0.1;
