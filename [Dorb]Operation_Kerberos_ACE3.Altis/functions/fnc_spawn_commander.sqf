...
If (!(worldName in ["pja305"])) then {
	...
	If (_rand < 3) then {SETVAR(_commander,DORB_COMMANDER_AI,true);};
};

// Arty
_rand = floor (random 4);
If (_rand < 3) then {SETVAR(_commander,DORB_COMMANDER_ART,true);};


// Tanks
_rand = floor (random 4);
If (_rand < 3) then {SETVAR(_commander,DORB_COMMANDER_TANKS,true);};


// Sniper
_rand = floor (random 4);
If (_rand < 3) then {SETVAR(_commander,DORB_COMMANDER_SNIPER,true);};
