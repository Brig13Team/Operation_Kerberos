...
private _min = getNumber(configfile >> "CfgKerberos" >> "mainmissions" >> _mainType >> "side" >> "min");
private _max = getNumber(configfile >> "CfgKerberos" >> "mainmissions" >> _mainType >> "side" >> "max");
private _amount = ceil (random(_max - _min)) + _min;

private _sidetypes = getArray(configfile >> "CfgKerberos" >> "mainmissions" >> _mainType >> "side" >> "sidemissions");
TRACEV_1(_sidetypes);
If ("%ALL" in _sidetypes) then {
    _sidetypes = configProperties [configfile >> "CfgKerberos" >> "sidemissions", "true", true];
}else{
    _sidetypes = _sidetypes select {isClass(configfile >> "CfgKerberos" >> "sidemissions" >> _x)};
    _sidetypes = _sidetypes apply {configfile >> "CfgKerberos" >> "sidemissions" >> _x};
};
TRACEV_1(_sidetypes);
