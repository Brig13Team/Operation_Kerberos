...
IF (_missionTargetType isEqualTo "") then {
    _allConfigs = configProperties [(ConfigFile >> "CfgKerberos" >> "CfgCompositions" >> "houses"),(format["getText(_x >> 'type') == '%1'",_housetype]),true];
}else{
    ...
    _allConfigs = configProperties [(ConfigFile >> "CfgKerberos" >> "CfgCompositions" >> "houses"),_condition,true];
};
