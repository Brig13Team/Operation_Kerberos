...
if (!isNumber(ConfigFile >> "CfgKerberos" >> "headquarter" >> "drones" >> typeOf _drone >> "scan_radius")) exitWith { _drone call _onExit; false };

_scan_radius = getNumber(ConfigFile >> "CfgKerberos" >> "headquarter" >> "drones" >> typeOf _drone >> "scan_radius");
_wp_type = getText (ConfigFile >> "CfgKerberos" >> "headquarter" >> "drones" >> typeOf _drone >> "scan_waypoint");
if (_wp_type == "LOITER") then {
    _radius = getNumber (ConfigFile >> "CfgKerberos" >> "headquarter" >> "drones" >> typeOf _drone >> "waypoints" >> _wp_type >> "radius");
} else {
    _radius = 0;
};
...
_dir = random 360;
_height = getNumber (ConfigFile >> "CfgKerberos" >> "headquarter" >> "drones" >> typeOf _drone >> "waypoints" >> _wp_type >> "height");
