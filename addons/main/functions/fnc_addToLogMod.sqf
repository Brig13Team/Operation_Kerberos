...
//    [{
        ...
        while {(count GVAR(logging))>0} do {
            ...
            If (IS_ARRAY(_scriptName)) then {
                diag_log text format["[Servermod] (%1) - %2: ~\%3:%4 [%5]  %6<<%7",_componentName,_logLevel,_fileName,_line,_frameNumber,(_scriptName select 0),(_scriptName select 1)];
            }else{
                diag_log text format["[Servermod] (%1) - %2: ~\%3:%4 [%5]  %6",_componentName,_logLevel,_fileName,_line,_frameNumber,_scriptName];
            };
            ...
            If (isServer&&hasinterface&&_logLevel in ["ERROR","WARNING","TRACE"]) then {
                systemchat format["[Servermod] (%1) - %2: ~\%3:%4 [%5]",_componentName,_logLevel,_fileName,_line,_frameNumber];
