...
_group = group _drone;
if (!isClass(ConfigFile >> "CfgKerberos" >> "headquarter" >> "drones" >> _class >> "waypoints" >> _type)) exitWith {
    LOG("no config entry");
    false
};
...
_height = getNumber (ConfigFile >> "CfgKerberos" >> "headquarter" >> "drones" >> _class >> "waypoints" >> _type >> "height");
_waypoint = _group addWaypoint [_position, 0];
_waypoint setWaypointType _type;
switch (_type) do {
    case "LOITER": {
        private ["_ltype","_radius","_height"];
        _ltype = getText (ConfigFile >> "CfgKerberos" >> "headquarter" >> "drones" >> _class >> "waypoints" >> _type >> "type");
        _radius = getNumber (ConfigFile >> "CfgKerberos" >> "headquarter" >> "drones" >> _class >> "waypoints" >> _type >> "radius");
    ...
    case "HOLD": {
        ...
        _distance = getNumber (ConfigFile >> "CfgKerberos" >> "headquarter" >> "drones" >> _class >> "waypoints" >> _type >> "distance");
        _height = getNumber (ConfigFile >> "CfgKerberos" >> "headquarter" >> "drones" >> _class >> "waypoints" >> _type >> "height");
        _random = random 360;
