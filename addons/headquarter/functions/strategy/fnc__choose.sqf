...
} forEach (configProperties [ConfigFile >> "CfgKerberos" >> "headquarter" >> "strategy", "!((toLower(configname _x)) in ['groundattack'])", true]);
...
// execute the strategies until the current value reaches zero
{
    ...
    private _stratCfg = (ConfigFile >> "CfgKerberos" >> "headquarter" >> "strategy" >> _strategytype);
