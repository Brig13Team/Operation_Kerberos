...
If !(isClass(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints")) exitWith {
...
_startKeypos = getArray(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _startKey >> "pos");
_targetKeypos = getArray(mConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _targetKey >> "pos");
_distance = _startKeypos distance2D _targetKeypos;
_Keydifferenz = getNumber(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> "raster") / 10;
...
while {_active} do {
    ...
    _nextValue = getNumber(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "XM");
    TRACEV_2(_nextKey,_nextValue);
    if (_nextValue > 0) then {
        private ["_nextPos","_value","_inside"];
        _nextPos = getArray(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "pos");
    ...
    /*XM*/
    _nextKey = format["%1_%2",_currentKeyX - _Keydifferenz,_currentKeyY];
    _nextValue = getNumber(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "XP");
    if (_nextValue > 0) then {
        private ["_nextPos","_value","_inside"];
        _nextPos = getArray(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "pos");
        _value = _currentWay + _currentDistance + (If (_evade) then {[_nextKey,_distance] call FUNC(waypoints_evade)}else{0});
    ...
    _nextValue = getNumber(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "YM");
    if (_nextValue > 0) then {
        private ["_nextPos","_value","_inside"];
        _nextPos = getArray(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "pos");
        _value = _currentWay + _currentDistance + (If (_evade) then {[_nextKey,_distance] call FUNC(waypoints_evade)}else{0});
    ...
    /*YM*/
    _nextKey = format["%1_%2",_currentKeyX,_currentKeyY - _Keydifferenz];
    _nextValue = getNumber(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "YP");
    if (_nextValue > 0) then {
        private ["_nextPos","_value","_inside"];
        _nextPos = getArray(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "pos");
        _value = _currentWay + _currentDistance + (If (_evade) then {[_nextKey,_distance] call FUNC(waypoints_evade)}else{0});
    ...
    _nextValue = getNumber(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "XMYM");
    if (_nextValue > 0) then {
        private ["_nextPos","_value","_inside"];
        _nextPos = getArray(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "pos");
        _value = _currentWay + _currentDistance + (If (_evade) then {[_nextKey,_distance] call FUNC(waypoints_evade)}else{0});
    ...
    /*XMYM*/
    _nextKey = format["%1_%2",_currentKeyX - _Keydifferenz,_currentKeyY - _Keydifferenz];
    _nextValue = getNumber(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "XPYP");
    if (_nextValue > 0) then {
        private ["_nextPos","_value","_inside"];
        _nextPos = getArray(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "pos");
        _value = _currentWay + _currentDistance + (If (_evade) then {[_nextKey,_distance] call FUNC(waypoints_evade)}else{0});
    ...
    /*XMYP*/
    _nextKey = format["%1_%2",_currentKeyX - _Keydifferenz,_currentKeyY + _Keydifferenz];
    _nextValue = getNumber(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "XPYM");
    if (_nextValue > 0) then {
        private ["_nextPos","_value","_inside"];
        _nextPos = getArray(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "pos");
        _value = _currentWay + _currentDistance + (If (_evade) then {[_nextKey,_distance] call FUNC(waypoints_evade)}else{0});
    ...
    /*XPYM*/
    _nextKey = format["%1_%2",_currentKeyX + _Keydifferenz,_currentKeyY - _Keydifferenz];
    _nextValue = getNumber(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "XMYP");
    if (_nextValue > 0) then {
        private ["_nextPos","_value","_inside"];
        _nextPos = getArray(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextKey >> "pos");
        _value = _currentWay + _currentDistance + (If (_evade) then {[_nextKey,_distance] call FUNC(waypoints_evade)}else{0});
...
#ifdef DEBUG_MARKER
{
    [getArray(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> (_x select 3) >> "pos"),format["%1_%2_%3",_x select 0,_x select 1,_x select 2]] call EFUNC(common,debug_marker_create);
}forEach _waypointsActivated;
{
    [getArray(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> (_x select 3) >> "pos"),format["%1_%2_%3",_x select 0,_x select 1,_x select 2],"ColorBlue"] call EFUNC(common,debug_marker_create);
...
while {_active} do {
    ...
    If !(_nextWP isEqualTo "") then {
        ...
        #ifdef DEBUG_MARKER
            [getArray(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _nextWP >> "pos"),"","ColorBlack","hd_objective"] call EFUNC(common,debug_marker_create);
        #endif
    };
};
...
{
    private ["_temp"];
    _temp = (getArray(ConfigFile >> "CfgKerberos" >> "worlds" >> worldName >> "wayoints" >> _x >> "pos"));
    TRACEV_1(_temp);
