...
class GVAR(statemachine_Taskmanager) {
    ...
    list = QUOTE([false] call FUNC(getAllMissions));
    skipNull = 1;

    class initial {
        onState = "";
        onStateEntered = "";
        onStateLeaving = "_this setVariable ['progress', 'initializing'];";

        class toSpawn {
            targetState = "spawn";
            condition = "!((_this getVariable ['progress', 'none']) == 'cancel')";
            onTransition = QUOTE(_this setVariable [ARR_2('spawntimeout', CBA_missiontime + SPAWNTIMEOUT)]);
        };
    ...
    class spawn {
        onState = "";
        onStateEntered = QFUNC(spawnMission);
        onStateLeaving = "";
        class cancel {
            targetState = "cleanup";
            condition = "(_this getvariable ['progress', 'none'])=='cancel'";
    ...
    class cleanup {
        onStateEntered = QFUNC(cleanupMission);
        class toDump {
            targetState = "dump";
            condition = "true";
        };
    ...
    class dump {
        onStateEntered = QUOTE([_this] call CBA_fnc_deleteEntity);
    };
