...
If !(hasInterface) then {
    [QGVAR(teamkill), {
        ...
        diag_log format['[MissionFile] (COMPONENT) TEAMKILL(%1): %2 | %3  killed by  %4 | %5',diag_frameNo,name _target,group _target,name _shooter,group _shooter];
    }] call CBA_fnc_addEventHandler;

    [QGVAR(teamfire), {
        _this params [["_target",objNull,[objNull]],["_shooter",objNull,[objNull]],["_damage",-1,[0]]];
        diag_log format['[MissionFile] (COMPONENT) TEAMFIRE(%1): %2 | %3  shot by  %4 | %5  Damage=%6',diag_frameNo,name _target,group _target,name _shooter,group _shooter,_damage];
    }] call CBA_fnc_addEventHandler;
}else{
    [QGVAR(teamkill), {
