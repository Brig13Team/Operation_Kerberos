/**
 * Author: Dorbedo
 * find an empty position for spawning of a composition
 *
 * Arguments:
 * 0: <ARRAY> centerposition
 * 1: <SCALAR> the center of the searching area
 * 2: <SCALAR> amount of positions
 * 3: <SCALAR> size of composition
 *
 * Return Value:
 * <ARRAY> array of possible positions extended by a 4th dimension, the direction
 *
 */
...
while {((count _possibleSpawnpositions)<_amount)&&(_errorcounter < (300+_amount))} do {
    ...
    // get a flat position
    private _spawnpos = [_tempPos,30,_radius,20,0.09] call EFUNC(common,pos_flatempty);

    // fallback search
    If ((_spawnpos isEqualTo [])&&(_errorcounter > 200)) then {
        _spawnpos = [_tempPos,30,_radius,20,0.5] call EFUNC(common,pos_flatempty);
    };
