...
If (str(side player) == "GUER") then {
    ...
    [
        {
            (ace_player distance2D (getMarkerPos "respawn_guerrila")) < 300
...
[getMarkerPos "bay_north_sea",localize LSTRING(BAY_NORTH),"ship",{!(_this call FUNC(nearEnemys))},[getMarkerPos "bay_north_sea"]] call FUNC(registerTeleportTarget);
